...
//    {
//        // Get meeting statuses and counts directly from DB table
//        $counts = DB::table('meetings')
//            ->select('status', DB::raw('count(*) as count'))
//            ->groupBy('status')
//            ->pluck('count', 'status')  // returns [status => count]
//            ->toArray();
//
...
//            $percentages[$status->value] = $total > 0 ? round(($count / $total) * 100, 2) : 0;
//        }
//
//        // Convert current date to Jalali formatted string
...
//        $completedTasks = DB::table('task_users')
//            ->where('task_status', TaskStatus::SENT_TO_SCRIPTORIUM->value)
//            ->whereColumn('sent_date', '<=', 'time_out')
//            ->count();
//
//        // Completed tasks with delay
//        $completedTasksWithDelay = DB::table('task_users')
//            ->where('task_status', TaskStatus::SENT_TO_SCRIPTORIUM->value)
//            ->whereColumn('sent_date', '>', 'time_out')
//            ->count();
//
//        // Incomplete tasks on time
//        $incompleteTasks = DB::table('task_users')
//            ->where('task_status', TaskStatus::PENDING->value)
//            ->whereNull('sent_date')
//            ->where('time_out', '>=', $now)
//            ->count();
//
//        // Incomplete tasks with delay
//        $incompleteTasksWithDelay = DB::table('task_users')
//            ->where('task_status', TaskStatus::PENDING->value)
//            ->whereNull('sent_date')
//            ->where('time_out', '<=', $now)
//            ->count();
