                ...
                <div x-data="{ openMeeting: false }" class="w-full">
                    ...
                    <div x-show="openMeeting" x-transition class="mt-1 space-y-1 pr-3">
                        <x-link.responsive-link href="{{route('meeting.create')}}"
                                                :active="request()->is('create/new/meeting')" class="flex items-center gap-x-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                            {{__('ایجاد جلسه جدید')}}
                        </x-link.responsive-link>
                        <x-link.responsive-link href="{{route('my.task.table')}}"
                                                :active="request()->is('my/task/table')" class="flex items-center gap-x-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4"><path stroke-linecap="round" stroke-linejoin="round" d="M6 6.878V6a2.25 2.25 0 0 1 2.25-2.25h7.5A2.25 2.25 0 0 1 18 6v.878m-12 0c.235-.083.487-.128.75-.128h10.5c.263 0 .515.045.75.128m-12 0A2.25 2.25 0 0 0 4.5 9v.878m13.5-3A2.25 2.25 0 0 1 19.5 9v.878m0 0a2.246 2.246 0 0 0-.75-.128H5.25c-.263 0-.515.045-.75.128m15 0A2.25 2.25 0 0 1 21 12v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6c0-.98.626-1.813 1.5-2.122" /></svg>
                            {{__('اقدامات من')}}
                        </x-link.responsive-link>
            ...
            <li class="mb-2">
                <div x-data="{ openMessage: false }" class="w-full">
                    <button @click="openMessage = !openMessage"
                            class="w-full flex items-center justify-between px-4 py-2 font-medium text-gray-300 hover:bg-gray-700 hover:text-white transition ease-in-out duration-300 rounded-md relative">
                        <div class="flex items-center gap-2">
                            @if(\App\Models\Notification::where('recipient_id',auth()->user()->id)->where('read_at',null)->exists())
                    ...
                    <div x-show="openMessage" x-transition class="mt-1 space-y-1 pr-3">
                        <x-link.responsive-link href="{{route('received.message')}}"
                            ...
                            <span class="flex justify-between w-full">
                               ...
                               {{($count = \App\Models\Notification::where('recipient_id', auth()->user()->id)->whereNull('read_at')->count()) > 10 ? '10+' : $count}}
                            </span>
                        </x-link.responsive-link>
                        <x-link.responsive-link href="{{route('sent.message')}}"
                                                :active="request()->is('sent/message')" class="flex items-center gap-x-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                 stroke="currentColor" class="size-4">
                                <path stroke-linecap="round"
                                      stroke-linejoin="round"
