<x-guest-layout>
    ...
    <form action="{{route('register.store')}}" method="post" dir="rtl">
        @csrf
        <p class="mx-auto mb-4">{{__('فرم ثبت نام')}}</p>
        <!-- Full Name -->
        <div>
            <x-input-label for="full_name" :value="__('نام و نام خانوادگی')" />
            <x-text-input name="full_name" class="mt-2" autofocus/>
            <x-input-error :messages="$errors->get('full_name')" class="mt-2" />
        </div>
        <!-- Personal Code -->
        <div>
            <x-input-label for="p_code" :value="__('کدپرسنلی')" class="mt-2"/>
            <x-text-input name="p_code" class="mt-2" maxlength="6"  autofocus/>
            <x-input-error :messages="$errors->get('p_code')" class="mt-2" />
        </div>
        <!-- Password -->
        <div x-data="{ show: true }">
            <x-input-label for="password" class="mt-2" :value="__('رمز')" />
            <div class="relative mt-2">
                <input name="password" maxlength="8"  autofocus
        ...
        <!-- Confirm Password -->
        <div x-data="{ show: true }">
            <x-input-label for="password_confirmation" class="mt-2" :value="__('تکرار رمز')" />
            <div class="relative mt-2">
                <input name="password_confirmation" maxlength="8" autofocus
                       class="w-full text-sm bg-white border rounded-md border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50"
                       :type="show ? 'password' : 'text'" type="password">
                <div class="absolute top-1/2 left-4 cursor-pointer" style="transform: translateY(-50%);">
                    <svg class="h-4 text-gray-700 block" fill="none" @click="show = !show" :class="{'hidden': !show, 'block':show }" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
