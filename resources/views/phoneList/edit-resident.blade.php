...
<x-app-layout>
    ...
    @can('has-permission-and-role', [UserPermission::PHONE_PERMISSIONS->value, UserRole::ADMIN->value])
        <form action="{{ route('resident-phones.update', $record->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="max-w-5xl p-6 bg-white shadow-lg rounded-2xl space-y-8 font-sans pb-6">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ __('ویرایش اطلاعات ساکن') }}</h2>
