<div>
    ...
    <div class="max-w-4xl mx-auto p-6 space-y-6" dir="rtl">
        ...
        <x-search-input>
            <x-text-input type="text" id="search" wire:model.live="search" class="block ps-10"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            @foreach($this->organizationUsers as $organizationUser)
                @foreach($this->organizations->where('id', $organizationUser->organization_id) as $organization)
                    <div class="bg-[#FCF7F8] text-black shadow-lg p-5 flex flex-col items-center text-center rounded-xl">
                        @if($organization->image)
                            <img src="{{$organization->getImageUrl()}}" class="w-20 h-20 rounded-full object-cover mb-4" alt="">
                        @endif
