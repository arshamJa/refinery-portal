...
{{--    @foreach(['create' => 'ایجاد سامانه جدید', 'import' => 'انتخاب فایل Excel', 'delete' => 'حذف سامانه', 'update' => 'ویرایش سامانه'] as $modalType => $title)--}}
{{--        <x-modal name="{{ $modalType }}">--}}
{{--            @if($modalType !== 'delete' || $organizationId)--}}
...
{{--                            <div class="w-full">--}}
{{--                                @if(in_array($modalType, ['create', 'update']))--}}
{{--                                    <x-input-label for="organization" :value="__('سامانه')"/>--}}
{{--                                    <x-text-input wire:model="organization" id="organization" class="block my-2 w-full" type="text" autofocus/>--}}
...
{{--                                    <x-text-input wire:model="image" id="image" class="block my-2 p-2 w-full" type="file"/>--}}
...
{{--                                <div class="mx-auto flex items-center justify-center size-12 rounded-full bg-red-100 sm:mx-0 sm:size-10">--}}
...
{{--                                        {{ __('آیا مطمئن هستید که ') }} <span class="font-medium">{{$organization}}</span> {{__('پاک شود ؟')}}--}}
{{--                                    </h3>--}}
{{--                                </div>--}}
{{--                            </div>--}}
{{--                        </div>--}}
        ...
        <x-modal name="create">
            <form wire:submit="createOrg" enctype="multipart/form-data">
                <div class="flex flex-row justify-end px-6 py-4 bg-gray-100 text-start">
                    {{__('ایجاد سامانه جدید')}}
                </div>
                <div class="px-6 py-4" dir="rtl">
                    <div class="mt-4 text-sm text-gray-600">
                        <div class="w-full">
                            <x-input-label for="organization" :value="__('سامانه')"/>
                            <x-text-input wire:model="organization" id="organization"
                            ...
                            <x-input-label for="image" :value="__('آپلود عکس')"/>
                            <x-text-input wire:model="image" id="image"
...
{{--                    @can('create-department-organization')--}}
                        ...
                        {{--                                </x-secondary-button>--}}
                        {{--                            </a>--}}
                        {{--                            <x-secondary-button wire:click="openImportModal">--}}
                        {{--                                Import--}}
                        {{--                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"--}}
                        {{--                                     stroke-width="1.5"--}}
        ...
        <div class="pt-4 sm:px-10 sm:pt-6 border shadow-md rounded-md">
            <table wire:loading.class.delay="opacity-70"
                ...
                <tr>
                    ...
                    <th class="px-4 py-3">{{__('سامانه')}}</th>
                    <th class="px-4 py-3">{{__('لینک سامانه')}}</th>
                    {{-- <th class="px-4 py-3 text-start">{{__('لوگو')}}</th>--}}
                ...
                @forelse($this->organizations as $organization)
                    <tr class="px-4 py-3 border-b text-center"
                        ...
                        <td class="px-4 py-4 whitespace-no-wrap text-sm leading-5 text-coll-gray-900">{{$organization->organization_name}}</td>
                        <td class="px-4 py-4 whitespace-no-wrap text-sm leading-5 text-coll-gray-900">
                            {{$organization->url}}
                        </td>
                        <td class="px-4 py-4 whitespace-no-wrap flex flex-row justify-center gap-x-2 text-sm leading-5">
{{--                            @can('update-department-organization')--}}
                                <x-primary-button class="flex gap-x-1"
                                                  wire:click="openModalEdit({{$organization->id}})">
                                    {{__('ویرایش')}}
