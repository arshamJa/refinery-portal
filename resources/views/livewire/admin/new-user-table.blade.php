                                    ...
                                    <tr class="px-4 py-3 border-b text-center" wire:key="{{$userInfo->id}}">
                                        ...
                                        <td class="px-4 py-4 whitespace-no-wrap flex flex-row gap-x-2 text-sm leading-5 text-coll-gray-900">
                                            <x-dropdown>
                                                ...
                                                <x-slot name="content">
                                                    @can('view-user')
                                                        <x-dropdown-link href="{{Illuminate\Support\Facades\URL::signedRoute('newUser.show',$userInfo->id)}}">
                                                            {{__('نمایش')}}
                                                        </x-dropdown-link>

                                                    @endcan
                                                    @can('update-user')
                                                        <x-dropdown-link href="{{Illuminate\Support\Facades\URL::signedRoute('newUser.edit',$userInfo->id)}}">
                                                            {{__('ویرایش')}}
                                                        </x-dropdown-link>
                                                    @endcan
                                                    @can('delete-user')
                                                        <button wire:click="openModalDelete({{$userInfo->id}})" class="block w-full px-4 py-2 text-start text-sm leading-5 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-800 transition duration-150 ease-in-out">
                                                            {{__('حذف')}}
                                                        </button>
                                                    @endcan
                                                </x-slot>
