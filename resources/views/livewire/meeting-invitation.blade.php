...
{{--            <x-template>--}}
                ...
                <div wire:poll.visible.60s>
                    ...
                    <div class="space-y-5">
                        <div class="bg-teal-50 rounded-lg p-4 dark:bg-teal-800/30">
                            <div class="flex">
                                ...
                                <div class="ms-3 w-full">
                                    ...
                                    <div class="border-t-2 mt-2 p-2" dir="rtl">
                                    @if($meetingUser->where('meeting_id',$meetingUser->meeting->id)->where('user_id',auth()->user()->id)->value('is_present') == '-1')
                                        <form action="{{route('sendInvitation',$meetingUser->meeting->id)}}" method="post">
                                            @csrf
                                            <p>{{__('در صورت انتخاب جانشین فیلد زیر را پر کنید :')}}</p>
                                                <input type="checkbox" name="checkBox" id="myCheckBox{{$meetingUser->id}}"  @checked(old('checkBox'))>
                                                <span>
                                                    {{__('در جلسه نمیتوانم شرکت کنم ولی جانشین این جانب، آقا/خانم')}}
                                                    <input type="text" name="full_name" value="{{old('full_name')}}" placeholder="نام و نام خانوادگی" class="w-60 mx-2 text-sm bg-white border rounded-md border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50">
                                                </span>
                                                <span>
                                                    {{__('و شماره پرسنلی')}}
                                                    <input type="text" name="p_code" value="{{old('p_code')}}" placeholder="شماره پرسنلی"  class="w-40 mr-2 ml-3 text-sm bg-white border rounded-md border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50">
                                                </span>
                                                <span>{{__('در جلسه مذکور شرکت می نماید')}}</span>
                                                <div class="mt-2">
                                                    <x-primary-button disabled id="submitButton{{$meetingUser->id}}" type="submit">{{__('ارسال دعوتنامه به جانشین')}}</x-primary-button>
                                                </div>
                                            @if ($errors->any())
                        ...
                        <script>
                            const checkBox{{$meetingUser->id}} = document.getElementById('myCheckBox{{$meetingUser->id}}');
                            const submitButton{{$meetingUser->id}} = document.getElementById('submitButton{{$meetingUser->id}}');
                            checkBox{{$meetingUser->id}}.addEventListener('change',function (){
                               submitButton{{$meetingUser->id}}.disabled = !this.checked;
                            });
                            submitButton{{$meetingUser->id}}.disabled = !checkBox{{$meetingUser->id}}.checked;
                        </script>
                    @endforeach
